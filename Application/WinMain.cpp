		...
		else
		{
			...
			// Render UI
			{
				...
				if(timerGraphRootNode)
				{
					...
					static bool(*imguiPrintTimerGraphRecurse)(const DxGpuPerformance::TimerGraphNode*, int, int) = [](const DxGpuPerformance::TimerGraphNode* node, int level, int targetLevel) -> bool
					{
						...
						if (level == targetLevel)
						{
							...
							if (node->mLastDurationMs > 0.0f)
							{
								...
								if (ImGui::IsItemHovered()) ImGui::SetTooltip(debugStr);
								ImGui::SameLine();
								ImGui::PopItemWidth();
							}
							printDone = true;
					...
					for (int targetLevel = 0; targetLevel < 16; ++targetLevel)
					{
						...
						if(printDone) ImGui::NewLine(); // start a new line if anything has been printed
					}
					ImGui::EndChild();
					ImGui::PopStyleVar(3);
