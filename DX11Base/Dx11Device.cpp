...
void Dx11Device::internalInitialise(const HWND& hWnd)
{
	...
	hr = D3D11CreateDeviceAndSwapChain(NULL, D3D_DRIVER_TYPE_HARDWARE, NULL, DX_DEBUG_EVENT|DX_DEBUG_RESOURCE_NAME ? D3D11_CREATE_DEVICE_DEBUG : NULL,
		requestedFeatureLevels, requestedFeatureLevelsCount, D3D11_SDK_VERSION, &scd, &mSwapchain, &mDev, NULL, &mDevcon);
	ATLASSERT(hr == S_OK);
